...
# Verifica se a pasta ../certificates existe, se não, cria
...
function generateCACertificate(){
    ...
    if [ -f "../certificates/root-ca.key" ] && [ ! -f "../certificates/root-ca.crt" ]; then
...
function generateCertificateSigningRequest(){
    if [ ! -f "../certificates/signing-request.key" ] && [ ! -f "../certificates/signing-request.csr" ]; then
...
function generateNginxCertificate(){
    if [ -f "../certificates/signing-request.csr" ] && [ -f "../certificates/root-ca.crt" ] && [ -f "../certificates/config.ext" ] && [ ! -f "../certificates/nginx-certificate.crt" ]; then
...
function generateKestrelCertificate(){
    if [ -f "../certificates/signing-request.key" ] && [ -f "../certificates/nginx-certificate.crt" ] && [ ! -f "../certificates/kestrel-certificate.pfx" ]; then
